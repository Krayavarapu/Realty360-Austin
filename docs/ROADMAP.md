...
1. **Phase 4.3** — `POST /predict/flip` rules engine
...
| Flip deal config | `shared/flip/types.ts`, `shared/flip/config.ts`, `shared/flip/deal-costs.ts`, `shared/flip/prediction-types.ts`, `shared/flip/prediction-schema.ts` |
